...
public class LicenseManager: ObservableObject {
    ...
    public var deviceId: String {
        return UIDevice.current.identifierForVendor?.uuidString ?? "IOS-EPZ-HWID"
    }
    ...
    private init() {
        self.savedKey = UserDefaults.standard.string(forKey: "epz_key") ?? ""
        self.isActivated = UserDefaults.standard.bool(forKey: "epz_act")
    ...
    public func validateKey(key: String, isAutoCheck: Bool = false, completion: @escaping (Bool, String) -> Void) {
        ...
        guard !cleanKey.isEmpty else {
            self.errorMessage = "Please enter key."
        ...
        URLSession.shared.dataTask(with: req) { [weak self] data, _, error in
            DispatchQueue.main.async {
                ...
                if let _ = error {
                    if isAutoCheck && self.isActivated { completion(true, "cached"); return }
                    self.errorMessage = "Cannot connect to Wispbyte Server (http://78.154.103.8:15429)."
                    completion(false, "net_err")
                    return
                }
                guard let data = data, let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else {
                    self.errorMessage = "Invalid server response."
                    completion(false, "json_err")
                    return
                }
                ...
                if isValid {
                    ...
                    UserDefaults.standard.set(cleanKey, forKey: "epz_key")
                    UserDefaults.standard.set(true, forKey: "epz_act")
                    completion(true, "ok")
                } else {
    ...
    public func logout() {
        self.isActivated = false
        self.savedKey = ""
        UserDefaults.standard.set(false, forKey: "epz_act")
    }
}
...
public class OverlayManager: ObservableObject {
    ...
    public func updateMetrics() {
        ...
        self.isThermalOverheating = (st == .serious || st == .critical)
